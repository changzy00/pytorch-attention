...
""" 
PyTorch implementation of KVT: k-NN Attention for Boosting Vision Transformers

As described in https://arxiv.org/abs/2106.00515

Specifically, instead of involving all the tokens for attention matrix calculation, 
KVT only selects the top-k similar tokens from the keys for each query to compute the 
attention map. The proposed k-NN attention naturally inherits the local bias of
CNNs without introducing convolutional operations, as nearby tokens
tend to be more similar than others.
"""
...
class KNNAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        mask = torch.zeros(B, self.num_heads, N, N)
        index = torch.topk(attn, k=self.topk, dim=-1)[1]
...
class TransformerEncoder(nn.Module):
    def __init__(self, dim, num_heads=4, mlp_ratio=4, qkv_bias=False, attn_drop=0, proj_drop=0, topk=100):
        super().__init__()
        hidden_features = int(dim * mlp_ratio)
        self.attn = KNNAttention(dim, num_heads, qkv_bias, attn_drop, proj_drop, topk)

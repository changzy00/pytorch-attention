...
# ------------------------------------------
""" 
PyTorch implementation of CSWin Transformer: A General Vision Transformer Backbone with Cross-Shaped

As described in https://arxiv.org/pdf/2107.00652.pdf

CSWin Transformer (the name CSWin stands for Cross-Shaped Window) is a new general-purpose backbone for 
computer vision. It is a hierarchical Transformer and replaces the traditional full attention with our 
newly proposed cross-shaped window self-attention. The cross-shaped window self-attention mechanism computes 
self-attention in the horizontal and vertical stripes in parallel that from a cross-shaped window, with each 
stripe obtained by splitting the input feature into stripes of equal width. 
"""
...
class CSWinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=96, depth=[2,2,6,2], split_size = [3,5,7],
                 ...
                 hybrid_backbone=None, norm_layer=nn.LayerNorm, use_chk=False):
        super().__init__()
